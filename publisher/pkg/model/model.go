...
type User struct {
	...
	Login     string `json:"login" db:"login" binding:"required" validate:"min=2,max=64"`
	Password  string `json:"password" db:"password" binding:"required" validate:"min=8,max=128"`
...
type Story struct {
	...
	Content  string    `json:"content" db:"content" binding:"required" validate:"min=4,max=2048"`
...
type Sticker struct {
	Id   int    `json:"id" db:"id"`
	Name string `json:"name" db:"name" binding:"required" validate:"min=4,max=2048"`
}
