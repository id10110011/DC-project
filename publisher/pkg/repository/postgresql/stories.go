...
func (r *StoriesPostgres) CreateStory(story model.Story) (model.Story, error) {
	var id int
	query := fmt.Sprintf("INSERT INTO %s (user_id, title, content, created, modified) values ($1, $2, $3, $4, $5) RETURNING id", storiesTable)
...
func (r *StoriesPostgres) UpdateStory(id int, story model.Story) (model.Story, error) {
	query := fmt.Sprintf(
		"UPDATE %s SET "+
			"id=$1, user_id=$2, title=$3, content=$4, created=$5, modified=$6 "+
			"WHERE id=$7",
		storiesTable)
