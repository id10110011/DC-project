...
func (r *UsersPostgres) CreateUser(user model.User) (model.User, error) {
	var id int
	query := fmt.Sprintf("INSERT INTO %s (login, password, firstname, lastname) values ($1, $2, $3, $4) RETURNING id", usersTable)
...
func (r *UsersPostgres) UpdateUser(id int, user model.User) (model.User, error) {
	query := fmt.Sprintf(
		"UPDATE %s SET "+
			"id=$1, login=$2, password=$3, firstname=$4, lastname=$5 "+
			"WHERE id=$6",
		usersTable)
